...
export default function CampusMap({ floorId, routeFloor, entranceMarker, transitionMarker, elevatorTransitionMarker, spiralTransitionMarker, presentationSegments, image, horizontalOnlyTerminal, hideRouteSegments, targetHighlight, specialDestination, classLabels = [] }) {
    ...
    <section className="campus-map" aria-label={`${floor.label} 校園地圖`}>
      <div aria-busy={!isMapLoaded} className="map-viewport" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}>
        <div className="map-canvas" style={{ transform: `translate(${view.x}px, ${view.y}px) scale(${view.scale})` }}>
          ...
          <RouteOverlay classLabels={classLabels} elevatorTransitionMarker={elevatorTransitionMarker} entranceMarker={entranceMarker} height={floor.height} hideRouteSegments={hideRouteSegments} horizontalOnlyTerminal={horizontalOnlyTerminal} presentationSegments={presentationSegments} routeFloor={routeFloor} specialDestination={isMapLoaded ? specialDestination : null} spiralTransitionMarker={spiralTransitionMarker} targetHighlight={isMapLoaded ? targetHighlight : null} transitionMarker={transitionMarker} width={floor.width} />
        </div>
